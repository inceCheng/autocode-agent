...
const createApp = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      // 跳转到对话页面，使用返回的 taskId、appId 和 token
      const { taskId, appId, token } = res.data.data
      if (taskId) {
        await router.push(`/app/chat/${taskId}?appId=${appId}&token=${token}`)
      } else {
        message.error('创建失败：未返回任务ID')
      }
    } else {
